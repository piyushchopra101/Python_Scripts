...
def performance1(all_jobs):
    ...
    makespan= max(x) - (min([i for i,e in enumerate(all_jobs) if e != 0]))




    return x,pick,average_hour_delay,average_hour_early,makespan,proc_time,v,idle_time,y
...
Jobs = collections.namedtuple("Jobs",['index', 'processing_time','allotted_time','due_date'])
...
for i in range( 0, T ):
    ...
    if number_jobs_this_period == 0:
        ...
        new_jobs = []
        nj=[]
        for j in range( 0, number_jobs_this_period ):
            new_jobs.append( GenJobFeat( job_proc_time, job_extra_time_allowed ) )
            due_date = i + new_jobs[j][1]

            job.append(Jobs(i,new_jobs[j][0],new_jobs[j][1],due_date))
        all_jobs.append( new_jobs )

print all_jobs
#print earliest_due_date(nj)
print performance1(all_jobs)
#names= index,processing_time,allotted_time,due_date,job
